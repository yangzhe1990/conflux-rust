...
class TestGetBestBlockHash(RpcClient):
    ...
    def test_two_chain(self):
        root = self.generate_block(1)

        # new 2 blocks
        blocks = self.generate_blocks(2, 1)
        ...
        f1 = self.generate_block_with_parent(root, [], 1)
        assert_equal(self.best_block_hash(), b2)

        # new block on another fork, then bigger hash one is the best block
        f2 = self.generate_block_with_parent(f1, [], 1)
        if f1 > b1:
        ...
        f3 = self.generate_block_with_parent(f2, [], 1)
