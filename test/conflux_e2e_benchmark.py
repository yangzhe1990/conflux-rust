...
class MessageTest(ConfluxTestFramework):
    def set_test_params(self):
        ...
        self.conf_parameters = {"log-level": "\"trace\"",
                                "storage-cache-start-size": "1000000",
                                "storage-cache-size": "20000000",
                                "storage-node-map-size": "200000000",
                                "ledger-cache-size": "16"}
    ...
    def run_test(self):
        ...
        for encoded in RlpIter(f):
            ...
            if 1.0 * tx_count / 1000 - elapsed_time >= 1:
                self.log.info("elapsed time %s, tx_count %s", elapsed_time, tx_count)
                time.sleep(1.0 * tx_count / 1000 - elapsed_time )
...
class BlockGenThread(threading.Thread):
    ...
    def run(self):
        while not self.stopped:
            try:
                h = self.node.generateoneblock(1000)
                time.sleep(1)
