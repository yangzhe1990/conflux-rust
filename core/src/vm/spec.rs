...
impl Spec {
    ...
    /// Spec for the Byzantium fork of the Ethereum main net.
    pub fn new_byzantium() -> Spec {
        let mut spec = Self::new_post_eip150(24576, true, true, true);
    ...
    /// Spec for the Constantinople fork of the Ethereum main net.
    pub fn new_constantinople() -> Spec {
        let mut spec = Self::new_byzantium();
        spec.have_bitwise_shifting = true;
        spec
    }

    fn new(efcd: bool, hdc: bool, tcg: usize) -> Spec {
        Spec {
...
#[test]
#[cfg(test)]
fn spec_evm_assumptions() {
    let s1 = Spec::new_frontier();
    let s2 = Spec::new_homestead();
