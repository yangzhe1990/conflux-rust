    ...
    ) -> vm::Result<InstructionRequirements<Gas>>
    {
        ...
        let cost = match instruction {
            ...
            instructions::SSTORE => {
                ...
                let gas = if spec.eip1283 {
                    let orig =
                        U256::from(&*context.initial_storage_at(&address)?);
                    calculate_eip1283_sstore_gas(spec, &orig, &val, &newval)
                } else {
...
fn calculate_eip1283_sstore_gas<Gas: evm::CostType>(
    ...
    Gas::from(if current == new {
        ...
        if original == current {
            // 2.1. If original value equals current value (this storage slot
            // has not been changed by the current execution context)
            if original.is_zero() {
            ...
            // 2.2. If original value does not equal current value (this
            // storage slot is dirty), 200 gas is deducted. Apply both of the
            // following clauses.
            spec.sload_gas

            // 2.2.1. If original value is not 0
            // 2.2.1.1. If current value is 0 (also means that new value is not
            // 0), remove 15000 gas from refund counter. We can prove that
            // refund counter will never go below 0. 2.2.1.2. If
            // new value is 0
            // (also means
            // that current
            // value is not
            ...
            // 2.2.2. If original value equals new value (this storage slot is
            // reset) 2.2.2.1. If original value is 0, add 19800
            // gas to refund counter. 2.2.2.2. Otherwise, add 4800
    ...
    if current == new {
        ...
        if original == current {
            ...
            if original.is_zero() {
                ...
                // 2.1.2. Otherwise, 5000 gas is deducted.
                if new.is_zero() {
                    // 2.1.2.1. If new value is 0, add 15000 gas to refund
                    // counter.
                    context.add_sstore_refund(sstore_clears_spec);
                }
            }
        } else {
            // 2.2. If original value does not equal current value (this
            ...
            if !original.is_zero() {
                // 2.2.1. If original value is not 0
                if current.is_zero() {
                    // 2.2.1.1. If current value is 0 (also means that new
                    // value is not 0), remove 15000 gas from refund counter.
                    // We can prove that refund counter will never go below 0.
                    context.sub_sstore_refund(sstore_clears_spec);
                } else if new.is_zero() {
                    // 2.2.1.2. If new value is 0 (also means that current
            ...
            if original == new {
                // 2.2.2. If original value equals new value (this storage slot
                // is reset)
                if original.is_zero() {
                    // 2.2.2.1. If original value is 0, add 19800 gas to refund
                    // counter.
                    let refund = context.spec().sstore_set_gas
                        - context.spec().sload_gas;
                    context.add_sstore_refund(refund);
                } else {
                    // 2.2.2.2. Otherwise, add 4800 gas to refund counter.
                    let refund = context.spec().sstore_reset_gas
                        - context.spec().sload_gas;
                    context.add_sstore_refund(refund);
                }
            }
